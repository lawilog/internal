...
class vector4d
{
	private:
		...
		{
			if(i >= n[0]) throw std::out_of_range(strprintf("vector4d: first index (which is %u) out of range (%u)", i, n[0]));
			if(j >= n[1]) throw std::out_of_range(strprintf("vector4d: second index (which is %u) out of range (%u)", j, n[1]));
			if(k >= n[2]) throw std::out_of_range(strprintf("vector4d: third index (which is %u) out of range (%u)", k, n[2]));
			if(l >= n[3]) throw std::out_of_range(strprintf("vector4d: fourth index (which is %u) out of range (%u)", l, n[3]));
		}
